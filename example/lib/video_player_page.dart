...
class VideoPlayerPage extends StatefulWidget {
  final bool autoPlay;
  const VideoPlayerPage({super.key, this.autoPlay = false});
...
class _VideoPlayerPageState extends State<VideoPlayerPage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop: true,
      onPopInvokedWithResult: (bool pop, result) async {
        if (_isFullScreen) {
    ...
    if (!_isVideoLoaded) {
      Future.delayed(Duration.zero, () async {
        await _viewPlayerController.loadVideo(
          videoUrl: 'https://cdn.deinerstertag.de/video/AWS-Elektroniker_in_Betriebstechnik-VR-GB-F04/HLS/master.m3u8',
        );
        if (widget.autoPlay) {
          await playAndPause();
        }
